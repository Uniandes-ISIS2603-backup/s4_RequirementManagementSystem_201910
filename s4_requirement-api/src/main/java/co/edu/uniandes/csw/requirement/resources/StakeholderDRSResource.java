...
public class StakeholderDRSResource {
     @Inject
    private StakeholderDRSLogic stakeholderDrsLogic; // Variable para acceder a la lógica de la aplicación. Es una inyección de dependencias.
     ...
     */
    @GET
    public DrsDTODetail getDrs(@PathParam("stakeholdersId") Long reqsId) {
        DRSEntity drsEntity = stakeholderDrsLogic.getDrs(reqsId);
        if (drsEntity == null) {
            throw new WebApplicationException("El recurso /stakeholdersId/" + reqsId + "/drs no existe.", 404);
        }
